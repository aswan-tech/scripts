...
try{
	...
	foreach ($_products as $key => $_product) {
		...
		foreach($confAtts as $productAttribute) {
		...
		foreach($childProducts as $child) { 
			...
			if((int)$qty) {
				$availableSizes[] = $attributeOptions['Size'][$child->getSize()];
			}
		 }		
		...
		if($totalQty > 2){
			...
			$_productdata['g:mrp'] = number_format($product->getPrice(), 2, null, '');
			$_productdata['g:price'] = number_format($fprice, 2, null, '');
			//$_productdata['g:color'] = isset($attributeOptions['Color']) ? implode($attributeOptions['Color'], ",") : null;
			//$_productdata['g:listed_size'] = is_array($attributeOptions['Size']) ? implode($attributeOptions['Size'], ",") : null;
			//$_productdata['g:available_sizes'] = is_array($availableSizes) ? implode($availableSizes, ",") : null;
			...
			$_productdata['g:product_images_link'] = implode($productImg, ",");
			...
			$sizesNode = $doc->createElement( "g:sizes" );
			$productNode->appendChild( $sizesNode );
			foreach ($availableSizes as $tag => $value) {
				$sizeDataTag = $doc->createElement( "size" );
				$sizesNode->appendChild( $sizeDataTag );
				
				$valueTag = $doc->createCDATASection($value);
				$sizeDataTag->appendChild( $valueTag );
			}
			
			$colorsNode = $doc->createElement( "g:colours" );
			$productNode->appendChild( $colorsNode );
			foreach ($attributeOptions['Color'] as $tag => $value) {
				$colorDataTag = $doc->createElement( "colour" );
				$colorsNode->appendChild( $colorDataTag );
				
				$valueTag = $doc->createCDATASection($value);
				$colorDataTag->appendChild( $valueTag );
			}						
...
/*scripts/yebhiPromotionFeed.php?catid=105*/
