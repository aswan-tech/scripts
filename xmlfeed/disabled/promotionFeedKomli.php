...
try{
	...
	header('Content-Disposition: attachment; filename="'.$catName.$today.'-product-feed-komli.xml"');
	...
	foreach ($_products as $key => $_product) {
		...
		foreach ($cats as $category_id) {
			...
			if($_cat->getLevel() == 3 ){
				$_productdata['sub_category_id_'.$j] = $_cat->getId();
				$_productdata['sub_category_name_'.$j] = $_cat->getName();
				$j++;
			}
			if($_cat->getLevel() == 2){
