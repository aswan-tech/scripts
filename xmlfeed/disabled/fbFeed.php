...
try{
	...
	header('Content-Disposition: attachment; filename="'.$catName.$today.'-fbstore-feed.xml"');
	...
	foreach ($_products as $key => $_product) {
		...
		$proGender = '';$proSize = ''; $proSizeName = '';$proColor = '';   
		foreach($childProducts as $child) { 
			$proGender = $child->getAttributeText('gender');
			$proColor = $child->getAttributeText('color');
			$proSize .= $child->getAttributeText('size').',';
		 }
		$proSizeName = substr($proSize,0,strlen($proSize)-1);
		$_productdata['title'] = $product->getName();
		$_productdata['g:color'] = $proColor;
		$_productdata['g:size'] = $proSizeName;	
		$_productdata['g:gender'] = $proGender;					
		...
		$_productdata['category_id'] = $cat_id;
		$_productdata['category_name'] = $catName;
